...
function Session({ children, accent = false }: { children: ReactNode; accent?: boolean }) {
  return (
    <span
      className={`shrink-0 rounded-sm border px-4 py-2.5 font-mono text-xl whitespace-nowrap ${accent ? "border-accent bg-accent/10 font-bold text-accent-strong" : "border-line bg-surface text-ink"
        }`}
    >
      {children}
    </span>
...
function Swap({ strong = false, label }: { strong?: boolean; label?: string }) {
  ...
  return (
    <div className="relative h-0.5 min-w-10 flex-1">
      ...
      <span className={`absolute top-1/2 left-0 -translate-y-1/2 text-sm leading-none ${head}`}>◀</span>
      <span className={`absolute top-1/2 right-0 -translate-y-1/2 text-sm leading-none ${head}`}>▶</span>
      {label && (
        <span className="absolute bottom-1/2 left-1/2 mb-2 -translate-x-1/2 bg-code-surface px-2 text-base whitespace-nowrap text-accent">
          {label}
        </span>
      )}
    </div>
...
function Workspace({ name, sessions }: { name: string; sessions: [string, string, string] }) {
  return (
    <div className="relative rounded-sm border border-line bg-code-surface px-6 py-6">
      ...
      <p className="mb-2.5 font-mono text-base font-bold tracking-wider text-accent">{name}</p>
    ...
    <Slide {...props}>
      ...
      <ul className="mt-10 flex gap-6">
        {REQUIREMENT1_WANTS.map((w) => (
          <li key={w.no} className="flex-1">
            ...
            <br />
            <span className="text-base font-semibold text-muted">{w.text}</span>
          </li>
        ))}
      </ul>

      <Step className="flex gap-6 mt-10">
        ...
        <div className="relative flex w-32 flex-col gap-6">
          ...
          <span className="absolute top-1/2 -left-6 -translate-y-1/2 text-sm leading-none text-accent">◀</span>
      ...
      <Step>
        <Takeaway className="mt-8">
          ① ② は
          <span className="font-semibold text-muted">大抵のターミナルやマルチプレクサで実現できます。③も多くが持ちますが、</span>
